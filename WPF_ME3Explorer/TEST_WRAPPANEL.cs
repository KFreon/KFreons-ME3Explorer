...
namespace WPF_ME3Explorer
{
    ...
    public class VirtualizingWrapPanel : VirtualizingPanel, IScrollInfo
    {
        ...
        private void ComputeExtentAndViewport(Size pixelMeasuredViewportSize, int visibleSections)
        {
            ...
            if (Orientation == Orientation.Horizontal)
            {
                _extent.Height = _abstractPanel.SectionCount + ViewportHeight - 1;// + Math.Abs(ViewportHeight - _abstractPanel.SectionCount);

                Console.WriteLine($"Extent height: {_extent.Height}");
                Console.WriteLine($"Viewport height: {ViewportHeight}");
                Console.WriteLine($"Section Count: {_abstractPanel.SectionCount}");
            }
            else
            {
        ...
        protected override void BringIndexIntoView(int index)
        {
            ...
            var currentVisibleMax = _offset.Y + ViewportHeight - ItemHeight;
            var itemsPerLine = Math.Max((int)Math.Floor(ViewportWidth / ItemWidth), 1);
            ...
            if (verticalOffsetRequiredToPutItemAtTopRow < currentVisibleMin) // if item is above visible area put it on the top row
                ...
                SetVerticalOffset(verticalOffsetRequiredToPutItemAtTopRow - ViewportHeight + ItemHeight);
        }
