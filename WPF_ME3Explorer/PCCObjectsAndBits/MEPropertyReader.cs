...
namespace WPF_ME3Explorer.PCCObjectsAndBits
{
    ...
    public static class SaltPropertyReader
    {
        ...
        public static CustomProperty PropertyToGrid(Property p, PCCObject pcc)
        {
            ...
            switch (p.TypeVal)
            {
                ...
                case Type.ObjectProperty:
                    ...
                    ppo.name = pcc.GetNameEntry(p.Value.IntValue);
        ...
        public static List<Property> ReadProp(PCCObject pcc, byte[] raw, int start)
        {
            ...
            if (!pcc.IsName(type) || size < 0 || size >= raw.Length)
                return result;
            string tp = (MEtype == 3) ? pcc.Names[type] : pcc.GetNameEntry(type);
            switch (tp)
            {
                case "BoolProperty":
                    p = new Property();
                ...
                case "ByteProperty":
                    ...
                    p.offsetval = pos + 32;
                    v = new PropertyValue();
                    v.StringValue = (MEtype == 3) ? pcc.GetNameEntry(sname) : pcc.GetNameEntry(sname);
                    v.len = size;
                    if (MEtype == 3)
                    {
                        pos += 32;
    ...
    public static class PropertyReader
    {
        ...
        {
            if ((flags & 0x0200000000000000) != 0)  // UnrealFlags.EObjectFlags.HasStack
            {
                return 30;
            }
            int result = 8;
