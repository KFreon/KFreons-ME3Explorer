...
namespace AmaroK86.ImageFormat
{
    ...
    public class ImageFile
    {
        ...
        protected ImageFile(string fileName, byte[] data)
        {
            ...
            Stream stream;
            if (data != null)
                stream = UsefulThings.RecyclableMemoryManager.GetStream(data);
            else
    ...
    public class TGA : ImageFile
    {
        ...
        private void RLEDecompress()
        {
            ...
            {
                MemoryStream oldImgData = UsefulThings.RecyclableMemoryManager.GetStream(imgData);
                MemoryStream newImgData = UsefulThings.RecyclableMemoryManager.GetStream((int)(imgSize.width * imgSize.height * BPP));
                byte[] buffer;
